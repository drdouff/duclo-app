...
class ProduitStock extends Model
{
     ...
     */
    protected $fillable = ['produit_id','quantite','stock_total','unit_price','total_price','published','ref','alias'];


    public function produit(){
        return $this->belongsTo('App\Models\Produit','produit_id');
    }
    protected $table='ProduitStocks';
 ...
 //To generate a 15 character reference
    public function generateReference(){
        ...
        if(empty($this->attributes['ref'])){
            ...
            while (ProduitStock::where('ref',$token)->first() instanceof ProduitStock);
    ...
    public function generateAlias($name){
        ...
        if(empty($this->attributes['alias'])){
            ...
            }while(ProduitStock::where('alias',$alias)->first() instanceof ProduitStock);

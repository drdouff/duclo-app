...
class StatutCmde extends Model
{
    use HasFactory;
...
//To generate a 15 character reference
   public function generateReference(){
       ...
       if(empty($this->attributes['ref'])){
           ...
           while (StatutCmde::where('ref',$token)->first() instanceof StatutCmde);
   ...
   public function generateAlias($name){
       ...
       if(empty($this->attributes['alias'])){
           ...
           }while(StatutCmde::where('alias',$alias)->first() instanceof StatutCmde);
